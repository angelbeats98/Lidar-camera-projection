...
def load_from_bin(bin_path):
    #change bin file into line=x column=4 matrix
    obj = np.fromfile(bin_path, dtype=np.float32).reshape(-1, 4)
    # ignore reflectivity info
    return obj[:,:4]

# bin file -> numpy array
velo_points = load_from_bin('./dataset/velodyne_points/data/0000000604.bin')
...
image = cv2.imread('dataset/image_'+ mode +'/data/0000000604.png')
width, height = image.shape[1], image.shape[0]
#plt.imshow(image)
print(image.shape)
# cv2.imwrite("ours.jpg",image)
...
def depth_color(val, min_d=0, max_d=120):
    ...
    np.clip(val, min_d, max_d, out=val)  # max distance is 120m but usually not usual
...
v2c_filepath = './dataset/calib_velo_to_cam.txt'
...
def calib_velo2cam(filepath):
    ...
    with open(filepath, "r") as f:
        ...
        for line in file:
            #when meet : split, split time is 1
            (key, val) = line.split(':', 1)
            if key == 'R':
                #read from val split when meet " "
...
def velo3d_2_camera2d_points(points, v_fov, h_fov, vc_path, cc_path, mode='02'):
    ...
    # 定义偏差向量
    bias_rotation_vector = np.array([-0.006, 0.00, 0.0])
    bias_translation_vector =np.array([0.0, 0.09, 0.0])
    bias_translation_vector = bias_translation_vector.reshape(3,1)
    # 将偏差向量加到旋转矩阵中
    R_vc = R_vc + bias_rotation_vector
    T_vc = T_vc + bias_translation_vector
    print(R_vc)
    #print(T_vc)
...
ans, c_ = velo3d_2_camera2d_points(velo_points, v_fov=(-90, 90), h_fov=(-90, 90), vc_path=v2c_filepath, cc_path=c2c_filepath, mode=mode)
