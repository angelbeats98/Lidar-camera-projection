...
def vis_pointcloud(points, colors):
    """
    渲染显示雷达点云
    ...
    point_size = np.zeros(points.shape[0], dtype=np.float16) + 0.2
...
if __name__ == '__main__':
    ...
    bias_rotation_vector = np.array([0.01, 0.0, 0.01, 0.01])
   # extrinsic= extrinsic+ bias_rotation_vector


    point_in_image, mask = get_fov_mask(point_in_lidar, extrinsic, intrinsic, h, w-2)
    valid_points = point_in_lidar[mask]
    #point_in_image=cv2.cvtColor(point_in_image, cv2.COLOR_BGR2RGB)

    u=point_in_image[:,0]+1
    v=point_in_image[:,1]
    z=point_in_image[:,2]
    print(point_in_image,u,v,z,sep='\n')
    image = cv2.imread(image_path)
    plt.subplots(1, 1, figsize=(13, 8))
    plt.title('Velodyne points to camera image Result', fontdict={'size': 20})
    #plt.rcParams['ytick.labelsize'] = 20
    plt.imshow(image)
    plt.scatter([u], [v], c=[z], cmap='rainbow', alpha=1, s=10)
    plt.show()
    # 获取颜色
